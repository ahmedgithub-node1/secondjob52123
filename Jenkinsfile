...
pipeline{  
    ...
    parameters{
        ...
        string(name: 'DuckerHubUser', description: "name of the docker Application", defaultValue: 'ahmedahmed123')
    }
         ...
         when { expression { params.action == 'create' } }
            steps{
                script{
                    ...
                    dockerBuild("${params.ImageName}","${params.ImageTag}","${params.DuckerHubUser}")
               }
            }   
        }
         stage('Dockcer Image Scan: trivy '){
         when { expression { params.action == 'create' } }
            steps{
                script{
